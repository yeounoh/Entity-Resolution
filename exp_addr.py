...
import random
import simplejson
from estimator import chao92, qaChao92, fChao92, sChao92,nominal, vNominal,  sNominal, wChao92, bChao92, dChao92, goodToulmin, vGoodToulmin, remain_switch, gt_switch, gt_marginal, gt_remaining, extrapolation, extrapolation2, extrapolation3,vRemainSwitch, unseen, twoPhase, vRemainSwitch2, minTasks, minTasks2, minTasks3, sampleCoverage, minTasksToCleanAll, extrapolateFromSample
...
show_min_tasks = True
'''
for w in range(1,len(d[0])):
    if w%100 == 0:
        hist = numpy.sum(d[:,:w]==1,axis=1)
        f1 = numpy.sum(hist == 1)
        n = numpy.sum(hist)
        print 'sample cov %d:'%w, sampleCoverage(d[:,:w]), 'n=%f'%n, 'f1=%f'%f1
'''
